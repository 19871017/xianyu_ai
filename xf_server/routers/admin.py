...
def list_users(db: Session = Depends(get_db), _=Depends(get_current_admin)):
    ...
    return [{"id": u.id, "username": u.username, "is_admin": u.is_admin, "is_active": u.is_active, "created_at": u.created_at.isoformat()} for u in users]


@router.get("/devices")
def list_devices(db: Session = Depends(get_db), _=Depends(get_current_admin)):
    devices = db.query(Device).all()
    return [{"id": d.id, "license_id": d.license_id, "machine_id": d.machine_id, "device_name": d.device_name, "is_active": d.is_active, "activated_at": d.activated_at.isoformat(), "last_verified": d.last_verified.isoformat()} for d in devices]


@router.get("/licenses")
def list_licenses(db: Session = Depends(get_db), _=Depends(get_current_admin)):
    licenses = db.query(License).all()
    return [{"id": l.id, "license_key": l.license_key, "user_id": l.user_id, "machine_id": l.machine_id, "expires_at": l.expires_at.isoformat(), "is_active": l.is_active, "days": l.days, "created_at": l.created_at.isoformat(), "activated_at": l.activated_at.isoformat() if l.activated_at else None} for l in licenses]
